...
public class EventEntity
{
    ...
    /// <summary>
    /// Which calendar type(s) display this event. 0 = All (default, backward-compatible).
    /// Added after initial release — migrated via TryAddColumnAsync.
    /// </summary>
    public int EventScopeValue { get; set; } = (int)EventScope.All;

    [Ignore]
    public EventScope EventScope
    {
        get => (EventScope)EventScopeValue;
        set => EventScopeValue = (int)value;
    }

    /// <summary>
    /// Alias of the Google account this event came from.
    /// Empty for the primary (writable) account.
    /// Added after initial release — migrated via TryAddColumnAsync.
    /// </summary>
    [MaxLength(100)]
