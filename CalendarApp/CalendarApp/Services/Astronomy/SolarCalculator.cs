...
public class SolarCalculator
{
    ...
    private DateTime CalculateSunTime(DateTime date, double latitude, double longitude, bool isSunrise)
    {
        ...
        var cosHa = Math.Cos(ToRadians(90.833)) / (Math.Cos(latRad) * Math.Cos(decRad))
                    - Math.Tan(latRad) * Math.Tan(decRad);

        // Handle polar day/night: if cosHa outside [-1,1], sun never rises/sets
        if (double.IsNaN(cosHa) || cosHa < -1.0 || cosHa > 1.0)
        {
            // Fallback: return solar noon for polar regions (sun always up or always down)
            var solarNoonFallback = 720.0 - 4.0 * longitude - eqTime;
            int dayOffsetFallback = (int)Math.Floor(solarNoonFallback / 1440.0);
            solarNoonFallback -= dayOffsetFallback * 1440.0;
            var hFallback = (int)(solarNoonFallback / 60.0);
            var mFallback = (int)(solarNoonFallback % 60.0);
            hFallback = Math.Max(0, Math.Min(23, hFallback));
            mFallback = Math.Max(0, Math.Min(59, mFallback));
            return new DateTime(date.Year, date.Month, date.Day, hFallback, mFallback, 0, DateTimeKind.Utc)
                .AddDays(dayOffsetFallback);
        }
        ...
        // Convert to UTC DateTime, adjusting the calendar date when the longitude offset
        // causes the event to fall on an adjacent calendar day (e.g. sunset at western
        // longitudes is often past UTC midnight and belongs to the following day).
        int dayOffset = (int)Math.Floor(sunTime / 1440.0);
        sunTime -= dayOffset * 1440.0;
