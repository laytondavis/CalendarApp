...
public class LunarCalculator
{
    ...
    {
        // Calculate approximate lunation number
        var k = ApproximateLunationNumber(afterDate);

        // Find the next new moon
        var newMoon = CalculateNewMoon(Math.Ceiling(k));

        // If the calculated new moon is before afterDate, try the next one
        while (newMoon <= afterDate)
        {
            k++;
            newMoon = CalculateNewMoon(Math.Ceiling(k));
        }
    ...
    {
        var k = ApproximateLunationNumber(beforeDate);
        var newMoon = CalculateNewMoon(Math.Floor(k));

        // If the calculated new moon is after beforeDate, try the previous one
        while (newMoon >= beforeDate)
        {
            k--;
            newMoon = CalculateNewMoon(Math.Floor(k));
        }
